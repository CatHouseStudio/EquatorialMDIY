...
int64_t Pluse_Horizontal_Relative(float azimuth);
int64_t Pluse_Vertical_Relative(float altitude);
int64_t Pluse_Horizontal_Absolute(float azimuth);
int64_t Pluse_Vertical_Absolute(float altitude);

void Move_Horizontal(uint64_t hor);
void Move_Vertical(uint64_t ver);

void task_Move_Horizontal(void *parameters);
void task_Move_Absolute(void *parameters);
...
{
    // TODO Depend on Mechanical structure reduction ratio coefficient
    return (int64_t)(azimuth * Stepper_Horizontal_K);
}
