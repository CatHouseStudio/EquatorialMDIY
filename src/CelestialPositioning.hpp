...
// Convert h:m:s → hours
double hms_to_hours(int h, int m, double s)
{
    return h + m / 60.0 + s / 3600.0;
}

// Convert d:m:s → decimal degrees
double dms_to_degrees(int d, int m, double s)
{
...
void degrees_to_dms(double deg, int &d, int &m, double &s)
{
    int sign = (deg >= 0) ? 1 : -1;
    deg = fabs(deg);
...
void degrees_to_hms(double deg, int &h, int &m, double &s)
{
