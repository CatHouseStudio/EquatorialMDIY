...
MPU6050 mpu;
SemaphoreHandle_t semphr_tiltMutex;  // RTOS互斥锁
 ...
 void InitTiltFusion()
{
    ...
    axo /= 200; ayo /= 200; azo /= 200;
    gxo /= 200; gyo /= 200; gzo /= 200;
    last_time = millis();
}
 ...
 void updateTiltFusion()
{
    ...
    for (int i = 1; i < FilterWindow; i++) {
        aaxs[i - 1] = aaxs[i]; sum_roll += aaxs[i] * i;
        aays[i - 1] = aays[i]; sum_pitch += aays[i] * i;
        aazs[i - 1] = aazs[i]; sum_ztilt += aazs[i] * i;
    }
    ...
    Px += 0.0025f;
    float Kx = Px / (Px + Rx); agx += Kx * (aax - agx); Px *= (1 - Kx);

    Py += 0.0025f;
    float Ky = Py / (Py + Ry); agy += Ky * (aay - agy); Py *= (1 - Ky);

    Pz += 0.0025f;
    float Kz = Pz / (Pz + Rz); agz += Kz * (aaz - agz); Pz *= (1 - Kz);
}
